...
- ✅ Sudah capek kuliah bertahun-tahun
- ✅ Pengen cepat wisuda
- ✅ Mungkin sambil cari kerja
- ✅ Gak mau ribet-ribet
- ✅ **Belum tentu expert programming**
...
## 📋 APA YANG BERUBAH DARI PROPOSAL LAMA?

### ❌ Yang Dihapus (Terlalu Ribet):
- ~~3 benchmark berbeda~~ → **Cukup 1 benchmark**
- ~~15-20 paper untuk literature review~~ → **Cukup 5-7 paper**
- ~~Statistik kompleks (ANOVA, t-test, dll)~~ → **Cukup perbandingan grafik**
- ~~480 eksperimen~~ → **Cukup 12 eksperimen**
- ~~16 minggu timeline~~ → **8 minggu**

### ✅ Yang Tetap Ada (Supaya Tetap Informatika):
- ✅ Fokus ke algoritma paralel (bukan cuma CRUD)
- ✅ Ada perbandingan performa
- ✅ Ada eksperimen nyata
- ✅ Tetap ada kontribusi ilmiah

---

## 🎯 JUDUL BARU (Pilih Salah Satu)
...
## 📖 BAB I - PENDAHULUAN (VERSI LENGKAP!)
...
## 📚 BAB II - TINJAUAN PUSTAKA (VERSI SUPER LENGKAP!)
...
1. ABSTRACT (1 paragraf) → Cari tahu topik utama
2. INTRODUCTION paragraf terakhir → Cari tahu contribution
3. METHODOLOGY → Cari tahu metode yang dipakai
4. CONCLUSION → Cari tahu hasil akhir
...
Sequential: Cuci 1 baju → tunggu selesai → cuci baju ke-2 → dst
Batch Processing: Kumpulkan 100 baju → cuci sekaligus dalam 1x proses
...
- ✅ Predictable (hasil selalu sama sequence-nya)
- ✅ No race condition
- ✅ Debugging mudah

**Kekurangan:**
- ❌ Lambat untuk volume besar
- ❌ Resource tidak dimanfaatkan maksimal (CPU idle saat waiting I/O)
- ❌ Tidak scalable
...
Total waktu ≈ waktu_task_terlama + overhead
```
...
Thread 2:          [==GIL==]        [==GIL==]
↑ Thread bergantian pakai GIL → TIDAK ADA SPEEDUP!
...
↑ SPEEDUP SIGNIFIKAN! (Karena invoice generation = I/O-bound)
```

**Kelebihan Thread:**
- ✅ Lebih cepat untuk I/O-bound tasks
- ✅ Overhead kecil (lightweight)
- ✅ Share memory (mudah komunikasi antar thread)

**Kekurangan Thread:**
- ❌ GIL limiting untuk CPU-bound tasks (Python)
- ❌ Potensi race condition (butuh locking)
...
Interpretasi:
S = 1   → tidak ada speedup
S = 2   → 2x lebih cepat (parallel 2x lebih cepat dari sequential)
S = 4   → 4x lebih cepat
...
Interpretasi:
E = 100%  → Perfect (ideal speedup)
E = 75-90% → Sangat baik
E = 50-75% → Baik
E < 50%   → Kurang efisien (overhead terlalu besar)
```
...
## 🔬 BAB III - METODE PENELITIAN (SUPER DETAIL!)
...
```
□ Buat file invoice_generator.py (copy dari template)
□ Test generate 1 invoice
□ Buat file sequential_processor.py
□ Test process 10 invoice
□ Test process 100 invoice
□ Catat hasilnya
```
...
□ Sudah built-in di Python, gak perlu install
□ Buat file database.py untuk connection
□ Test insert & read data

OPSI B: Pakai PostgreSQL (Kalau mau lebih real)
□ Install PostgreSQL dari https://www.postgresql.org/
□ Buat database "invoice_db"
□ Buat tabel "invoices"
□ Test connection dari Python

OPSI C: Skip Database (PALING GAMPANG!)
□ Pakai dummy data aja (list of dict)
□ Gak usah pake database sama sekali
□ (Tetap valid untuk eksperimen!)
```
...
```
□ Buat file thread_processor.py (copy dari template)
□ Pahami cara kerja ThreadPoolExecutor
□ Test dengan 2 workers
□ Test dengan 4 workers
□ Test dengan 8 workers
□ Bandingkan hasil dengan sequential
```

**Hari 11-12: Debugging & Optimization**
```
□ Cek apakah semua invoice ter-generate dengan benar
□ Cek apakah ada race condition (seharusnya tidak ada)
...
```
□ Buat file run_benchmark.py (copy dari template)
□ Test jalankan via command line
□ Verifikasi hasil tersimpan ke CSV
□ Dry run semua eksperimen
```
...
□ Cek apakah ada data yang aneh (outlier)
□ Kalau ada yang aneh, ulangi eksperimen untuk data tersebut
□ Buat tabel hasil yang rapi
```
...
□ Kalau kurang bagus, edit parameter (size, color, label)
□ Buat grafik tambahan (bar chart, speedup chart)
```
...
□ Buat tabel rangkuman metrik
□ Tulis interpretasi awal
```

**Hari 25-27: Tulis BAB IV (Hasil & Pembahasan)**
```
□ Masukkan tabel hasil eksperimen
...
```
□ BAB I: Copy template dari panduan ini, sesuaikan dengan konteks
□ BAB II: Masukkan 5-7 paper yang sudah dicari, tulis narasi
□ BAB III: Copy prosedur dari panduan ini, sesuaikan dengan kondisi real
```

**Hari 31-32: Tulis BAB V & Abstrak**
```
□ BAB V: Tulis kesimpulan (jawab rumusan masalah)
□ BAB V: Tulis saran untuk penelitian lanjutan
□ Abstrak: Ringkas BAB I-V dalam 150-250 kata
```

**Hari 33-34: Formatting & Revisi**
```
□ Set margin, line spacing, font sesuai aturan kampus
□ Buat daftar isi otomatis
□ Buat daftar tabel & gambar
□ Rapikan daftar pustaka (pakai Mendeley/Zotero)
...
2. **Throughput (Θ):** Jumlah invoice per detik (invoice/s)
...
## 💻 TEMPLATE KODE LENGKAP (TINGGAL COPY-PASTE!)
...
## 📊 BAB IV - HASIL DAN PEMBAHASAN (DENGAN CONTOH!)
...
Terlihat bahwa peningkatan dari 2 workers ke 4 workers signifikan (1.78x → 3.22x), namun peningkatan dari 4 workers ke 8 workers tidak terlalu besar (3.22x → 3.46x).
...
## ✅ CHECKLIST LENGKAP (56 Hari Step-by-Step)
...
### Motivasi:
```
Hari 1:  "Ini akan lama..." 😰
Hari 14: "Kode udah jalan!" 😊
Hari 28: "Setengah jalan!" 💪
Hari 42: "Hampir selesai!" 😎
Hari 56: "SELESAI! WISUDA!" 🎉🎓
```
...
❌ "Harus publish journal!"
✅ "Tujuan gua cuma lulus S1!"

❌ "Takut salah..."
✅ "Trial & error is part of learning!"
...
✅ Penjelasan konsep dari NOL (GIL, Thread, Sequential, dll)
✅ Template BAB I-V lengkap
✅ Cara mencari & membaca paper (step-by-step!)
✅ Kode 80% sudah jadi (tinggal copy-paste!)
✅ Prosedur eksperimen detail (command-by-command!)
✅ Checklist 56 hari (day-by-day tasks!)
✅ Troubleshooting lengkap (10+ common problems!)
✅ Contoh interpretasi hasil
✅ Panduan persiapan sidang
